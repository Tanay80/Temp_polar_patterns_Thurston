...
def derivatives_hpc(t, y):
    ...
    c = 1
    kc = np.sqrt(-k) / (a_t * 2.56e-18 + eps)                                                                		  #Dimensionless qty, since numerator is in sec inverse
      
    calD = Ha + eps
    
    d_theta = - (1.0 * np.sqrt(-k) / (c * a_t + eps)) * np.cos(theta) * np.cos(phi) * (1.0 + (np.sin(theta) * np.cos(phi))**2)
    d_phi   = (1.0 * np.sqrt(-k) / (c * a_t + eps)) * np.sin(phi) * (1.0 + (np.sin(theta) * np.cos(phi))**2) / np.sin(theta)
    d_psi_real = 0
    d_psi_imag = 0
    ...
    dR0_0 = H * R0_0 + (2.0/3.0) * kc * R0_1 + 0.4 * H * tr_R0
    dI0_0 = H * I0_0 + (2.0/3.0) * kc * I0_1 + 0.4 * H * tr_I0 - tau * I0_0
    
    zetaR = -(R0_0 * dR0_0 + I0_0 * dI0_0)/(calD * (R0_0**2 + I0_0**2 + eps))
    zetaI = -(R0_0 * dI0_0 - dR0_0 * I0_0)/(calD * (R0_0**2 + I0_0**2 + eps))
    
    dR0_1 = - (0.6 * H * (zetaR * R0_1 - zetaI * I0_1) + kc * (-0.8 * R0_11 + 0.4 * R0_22 + 0.4 * R0_33)) - tau * R0_1
    dR0_2 = - (0.6 * H * (zetaR * R0_2 - zetaI * I0_2) + kc * (-1.2 * R0_12)) - tau * R0_2
    dR0_3 = - (0.6 * H * (zetaR * R0_3 - zetaI * I0_3) + kc * (-1.2 * R0_13)) - tau * R0_3
    dI0_1 = - (0.6 * H * (zetaR * I0_1 + zetaI * R0_1) + kc * (-0.8 * I0_11 + 0.4 * I0_22 + 0.4 * I0_33)) - tau * (2.0/3.0) * I0_1
    dI0_2 = - (0.6 * H * (zetaR * I0_2 + zetaI * R0_2) + kc * (-1.2 * I0_12)) - tau * (2.0/3.0) * I0_2
    dI0_3 = - (0.6 * H * (zetaR * I0_3 + zetaI * R0_3) + kc * (-1.2 * I0_13)) - tau * (2.0/3.0) * I0_3

    dR0_11 = - (0.666666666666667 * kc * R0_1) - tau * (0.9 * R0_11 + 0.3 * R2_11)
    dR0_12 = - (0.5 * kc * R0_2) - tau * (0.9 * R0_12 + 0.3 * R2_12)
    dR0_13 = - (0.5 * kc * R0_3) - tau * (0.9 * R0_13 + 0.3 * R2_13)
    dR0_21 = - (0.5 * kc * R0_2) - tau * (0.9 * R0_21 + 0.3 * R2_21)
    dR0_22 = - (-0.333333333333333 * kc * R0_1) - tau * (0.9 * R0_22 + 0.3 * R2_22)
    dR0_23 = 0.0 - tau * (0.9 * R0_23 + 0.3 * R2_23)
    dR0_31 = - (0.5 * kc * R0_3) - tau * (0.9 * R0_31 + 0.3 * R2_31)
    dR0_32 = 0.0 - tau * (0.9 * R0_32 + 0.3 * R2_32)
    dR0_33 = - (-0.333333333333333 * kc * R0_1) - tau * (0.9 * R0_33 + 0.3 * R2_33)

    dI0_11 = - (0.666666666666667 * kc * I0_1) - tau * I0_11
    dI0_12 = - (0.5 * kc * I0_2) - tau * I0_12
    dI0_13 = - (0.5 * kc * I0_3) - tau * I0_13
    dI0_21 = - (0.5 * kc * I0_2) - tau * I0_21
    dI0_22 = - (-0.333333333333333 * kc * I0_1) - tau * I0_22
    dI0_23 = 0.0 - tau * I0_23
    dI0_31 = - (0.5 * kc * I0_3) - tau * I0_31
    dI0_32 = 0.0 - tau * I0_32
    dI0_33 = - (-0.333333333333333 * kc * I0_1) - tau * I0_33

    dR2_11 = H * (zetaR * R2_11 - zetaI * I2_11) - H * (0.444444444444444 * R2_11 - 0.222222222222222 * R2_22 - 0.222222222222222 * R2_33) - tau * (0.2 * R0_11 + 0.4 * R2_11)
    dR2_12 = H * (zetaR * R2_12 - zetaI * I2_12) - H * (0.333333333333333 * R2_12 + 0.333333333333333 * R2_21) - tau * (0.2 * R0_12 + 0.4 * R2_12)
    dR2_13 = H * (zetaR * R2_13 - zetaI * I2_13) - H * (0.333333333333333 * R2_13 + 0.333333333333333 * R2_31) - tau * (0.2 * R0_13 + 0.4 * R2_13)
    dR2_21 = H * (zetaR * R2_21 - zetaI * I2_21) - H * (0.333333333333333 * R2_12 + 0.333333333333333 * R2_21) - tau * (0.2 * R0_21 + 0.4 * R2_21)
    dR2_22 = H * (zetaR * R2_22 - zetaI * I2_22) - H * (-0.222222222222222 * R2_11 + 0.444444444444444 * R2_22 - 0.222222222222222 * R2_33) - tau * (0.2 * R0_22 + 0.4 * R2_22)
    dR2_23 = H * (zetaR * R2_23 - zetaI * I2_23) - H * (0.333333333333333 * R2_23 + 0.333333333333333 * R2_32) - tau * (0.2 * R0_23 + 0.4 * R2_23)
    dR2_31 = H * (zetaR * R2_31 - zetaI * I2_31) - H * (0.333333333333333 * R2_13 + 0.333333333333333 * R2_31) - tau * (0.2 * R0_31 + 0.4 * R2_31)
    dR2_32 = H * (zetaR * R2_32 - zetaI * I2_32) - H * (0.333333333333333 * R2_23 + 0.333333333333333 * R2_32) - tau * (0.2 * R0_32 + 0.4 * R2_32)
    dR2_33 = H * (zetaR * R2_33 - zetaI * I2_33) - H * (-0.222222222222222 * R2_11 - 0.222222222222222 * R2_22 + 0.444444444444444 * R2_33) - tau * (0.2 * R0_33 + 0.4 * R2_33)

    dI2_11 = H * (zetaR * I2_11 + zetaI * R2_11) - H * (0.444444444444444 * I2_11 - 0.222222222222222 * I2_22 - 0.222222222222222 * I2_33) - tau * I2_11
    dI2_12 = H * (zetaR * I2_12 + zetaI * R2_12) - H * (0.333333333333333 * I2_12 + 0.333333333333333 * I2_21) - tau * I2_12
    dI2_13 = H * (zetaR * I2_13 + zetaI * R2_13) - H * (0.333333333333333 * I2_13 + 0.333333333333333 * I2_31) - tau * I2_13
    dI2_21 = H * (zetaR * I2_21 + zetaI * R2_21) - H * (0.333333333333333 * I2_12 + 0.333333333333333 * I2_21) - tau * I2_21
    dI2_22 = H * (zetaR * I2_22 + zetaI * R2_22) - H * (-0.222222222222222 * I2_11 + 0.444444444444444 * I2_22 - 0.222222222222222 * I2_33) - tau * I2_22
    dI2_23 = H * (zetaR * I2_23 + zetaI * R2_23) - H * (0.333333333333333 * I2_23 + 0.333333333333333 * I2_32) - tau * I2_23
    dI2_31 = H * (zetaR * I2_31 + zetaI * R2_31) - H * (0.333333333333333 * I2_13 + 0.333333333333333 * I2_31) - tau * I2_31
    dI2_32 = H * (zetaR * I2_32 + zetaI * R2_32) - H * (0.333333333333333 * I2_23 + 0.333333333333333 * I2_32) - tau * I2_32
    dI2_33 = H * (zetaR * I2_33 + zetaI * R2_33) - H * (-0.222222222222222 * I2_11 - 0.222222222222222 * I2_22 + 0.444444444444444 * I2_33) - tau * I2_33
...
if __name__ == "__main__":
    ...
    save_path = os.path.join(output_dir, "H3.png")
    ...
    plt.title(r'Evolution of fluctuation amplitudes in $\mathbb{H}^3$', fontsize=14)
    plt.legend()
    plt.grid(True, which="both", ls="--", alpha=0.5)
    
    amp_save_path = os.path.join(output_dir, "H3_ampl.png")
