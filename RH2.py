...
def derivatives_hpc(t, y):
    ...
    d_theta = (1.0 * np.sqrt(-k) / (c * a_t + eps)) * (np.sin(theta)**2) * np.cos(theta) * (np.cos(phi)**2) * np.sin(phi)
    d_phi   = -(1.0 * np.sqrt(-k) / (c * a_t + eps)) * np.sin(theta) * np.cos(phi) * (1.0 + (np.sin(phi)**2))
    d_psi_real = 0
    d_psi_imag = 0
    ...
    S_vec_real = (1.0/3.0) * (F * R0_1 - F * R0_2)
    S_vec_imag = (1.0/3.0) * (F * I0_1 - F * I0_2)

    S_Re = H * R0_0 + S_vec_real + 0.4 * H * Tr_R0
    S_Im = H * I0_0 + S_vec_imag + 0.4 * H * Tr_I0 - (1.0/L) * I0_0
    
    norm_N_sq = R0_0**2 + I0_0**2 + eps
    
    N_dot_D = R0_0 * D_real + I0_0 * D_imag
    N_cross_D = R0_0 * D_imag - I0_0 * D_real

    Den_R = calD * norm_N_sq + N_dot_D
    Den_I = N_cross_D
    
    inv_Den_sq = 1.0 / (Den_R**2 + Den_I**2 + eps)

    Num_R = -(R0_0 * S_Re + I0_0 * S_Im)
    Num_I = -(R0_0 * S_Im - I0_0 * S_Re)

    zeta_real = (Num_R * Den_R + Num_I * Den_I) * inv_Den_sq
    zeta_imag = (Num_I * Den_R - Num_R * Den_I) * inv_Den_sq
    # -------------------------------------------------------------------------------------------------------------------

    dR0_0 = S_Re + zeta_real * D_real - zeta_imag * D_imag
    dI0_0 = S_Im + zeta_real * D_imag + zeta_imag * D_real
    
    C_term_real = -0.8 * F * R0_21
    C_term_imag = -0.8 * F * I0_21
    
    dR0_1 = -(0.6 * H * R0_1 + C_term_real) - tau * R0_1
    dI0_1 = -(0.6 * H * I0_1 + C_term_imag) - tau * (2/3) * I0_1

    C_term_real = (0.4 * F * R0_11) + (-0.4 * F * R0_22)
    C_term_imag = (0.4 * F * I0_11) + (-0.4 * F * I0_22)

    dR0_2 = -(0.6 * H * R0_2 + C_term_real) - tau * R0_2
    dI0_2 = -(0.6 * H * I0_2 + C_term_imag) - tau * (2/3) * I0_2

    C_term_real = -0.4 * F * R0_23
    C_term_imag = -0.4 * F * I0_23

    dR0_3 = -(0.6 * H * R0_3 + C_term_real) - tau * R0_3
    dI0_3 = -(0.6 * H * I0_3 + C_term_imag) - tau * (2/3) * I0_3

    H_term_real = -(2/3) * F * R0_2
    H_term_imag = -(2/3) * F * I0_2
    
    dR0_11 = -H_term_real - tau * (0.9 * R0_11 + 0.3 * R2_11)
    dI0_11 = -H_term_imag - tau * I0_11

    H_term_real = 0.5 * F * R0_1
    H_term_imag = 0.5 * F * I0_1

    dR0_12 = -H_term_real - tau * (0.9 * R0_12 + 0.3 * R2_12)
    dI0_12 = -H_term_imag - tau * I0_12
    ...
    dR0_21 = -H_term_real - tau * (0.9 * R0_21 + 0.3 * R2_21)
    dI0_21 = -H_term_imag - tau * I0_21

    H_term_real = (1/3) * F * R0_2
    H_term_imag = (1/3) * F * I0_2

    dR0_22 = -H_term_real - tau * (0.9 * R0_22 + 0.3 * R2_22)
    dI0_22 = -H_term_imag - tau * I0_22
    ...
    dR0_33 = -H_term_real - tau * (0.9 * R0_33 + 0.3 * R2_33)
    dI0_33 = -H_term_imag - tau * I0_33

    K_diag = (4/9) * H
    K_off  = -(2/9) * H
    K_sym  = (1/3) * H

    K_sum_real = K_diag * R2_11 + K_off * R2_22 + K_off * R2_33
    K_sum_imag = K_diag * I2_11 + K_off * I2_22 + K_off * I2_33
    
    dR2_11 = H * (zeta_real * R2_11 - zeta_imag * I2_11) - K_sum_real - tau * (0.2 * R0_11 + 0.4 * R2_11)
    dI2_11 = H * (zeta_real * I2_11 + zeta_imag * R2_11) - K_sum_imag - tau * I2_11

    K_sum_real = K_sym * R2_12 + K_sym * R2_21
    K_sum_imag = K_sym * I2_12 + K_sym * I2_21
    
    dR2_12 = H * (zeta_real * R2_12 - zeta_imag * I2_12) - K_sum_real - tau * (0.2 * R0_12 + 0.4 * R2_12)
    dI2_12 = H * (zeta_real * I2_12 + zeta_imag * R2_12) - K_sum_imag - tau * I2_12

    K_sum_real = K_sym * R2_13 + K_sym * R2_31
    K_sum_imag = K_sym * I2_13 + K_sym * I2_31

    dR2_13 = H * (zeta_real * R2_13 - zeta_imag * I2_13) - K_sum_real - tau * (0.2 * R0_13 + 0.4 * R2_13)
    dI2_13 = H * (zeta_real * I2_13 + zeta_imag * R2_13) - K_sum_imag - tau * I2_13

    K_sum_real = K_sym * R2_12 + K_sym * R2_21
    K_sum_imag = K_sym * I2_12 + K_sym * I2_21

    dR2_21 = H * (zeta_real * R2_21 - zeta_imag * I2_21) - K_sum_real - tau * (0.2 * R0_21 + 0.4 * R2_21)
    dI2_21 = H * (zeta_real * I2_21 + zeta_imag * R2_21) - K_sum_imag - tau * I2_21

    K_sum_real = K_off * R2_11 + K_diag * R2_22 + K_off * R2_33
    K_sum_imag = K_off * I2_11 + K_diag * I2_22 + K_off * I2_33

    dR2_22 = H * (zeta_real * R2_22 - zeta_imag * I2_22) - K_sum_real - tau * (0.2 * R0_22 + 0.4 * R2_22)
    dI2_22 = H * (zeta_real * I2_22 + zeta_imag * R2_22) - K_sum_imag - tau * I2_22

    K_sum_real = K_sym * R2_23 + K_sym * R2_32
    K_sum_imag = K_sym * I2_23 + K_sym * I2_32

    dR2_23 = H * (zeta_real * R2_23 - zeta_imag * I2_23) - K_sum_real - tau * (0.2 * R0_23 + 0.4 * R2_23)
    dI2_23 = H * (zeta_real * I2_23 + zeta_imag * R2_23) - K_sum_imag - tau * I2_23

    K_sum_real = K_sym * R2_13 + K_sym * R2_31
    K_sum_imag = K_sym * I2_13 + K_sym * I2_31

    dR2_31 = H * (zeta_real * R2_31 - zeta_imag * I2_31) - K_sum_real - tau * (0.2 * R0_31 + 0.4 * R2_31)
    dI2_31 = H * (zeta_real * I2_31 + zeta_imag * R2_31) - K_sum_imag - tau * I2_31

    K_sum_real = K_sym * R2_23 + K_sym * R2_32
    K_sum_imag = K_sym * I2_23 + K_sym * I2_32

    dR2_32 = H * (zeta_real * R2_32 - zeta_imag * I2_32) - K_sum_real - tau * (0.2 * R0_32 + 0.4 * R2_32)
    dI2_32 = H * (zeta_real * I2_32 + zeta_imag * R2_32) - K_sum_imag - tau * I2_32

    K_sum_real = K_off * R2_11 + K_off * R2_22 + K_diag * R2_33
    K_sum_imag = K_off * I2_11 + K_off * I2_22 + K_diag * I2_33

    dR2_33 = H * (zeta_real * R2_33 - zeta_imag * I2_33) - K_sum_real - tau * (0.2 * R0_33 + 0.4 * R2_33)
    dI2_33 = H * (zeta_real * I2_33 + zeta_imag * R2_33) - K_sum_imag - tau * I2_33
...
if __name__ == "__main__":
    ...
    save_path = os.path.join(output_dir, "RH2.png")
    ...
    plt.title(r'Evolution of fluctuation amplitudes in $\mathbb{R} \times \mathbb{H}^2$', fontsize=14)
    plt.legend()
    plt.grid(True, which="both", ls="--", alpha=0.5)
    
    amp_save_path = os.path.join(output_dir, "RH2_ampl.png")
