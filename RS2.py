...
def derivatives_hpc(t, y):
    ...
    c = 1
    F = np.sqrt(k) / (c * a_t * 2.56e-18 + eps)                                                             		  #Dimensionless qty, since numerator is in sec inverse
    ...
    denom = c * a_t * sin_arg + eps
    
    term_cos_minus_1 = cos_arg - 1.0
    term_1_minus_cos = 1.0 - cos_arg
      
    calD = Ha + eps
    
    d_theta = -0.5 * ((1.0 * np.sqrt(k) * (np.cos(chi * np.sqrt(k) / c) - 1.0)) / (c * a_t * np.sin(chi * np.sqrt(k) / c) + eps)) * (np.sin(theta)**2) * np.cos(theta) * (np.sin(2*phi)**2)
    d_phi   = -0.25 * ((1.0 * np.sqrt(k) * (np.cos(chi * np.sqrt(k) / c) - 1.0)) / (c * a_t * np.sin(chi * np.sqrt(k) / c) + eps)) * np.sin(theta) * np.sin(4*phi)
    d_psi_real = 0
    d_psi_imag = 0
    ...
    D_real = H * R0_0 + (2.0/15.0) * H * Tr_R0
    D_imag = H * I0_0 + (2.0/15.0) * H * Tr_I0

    S_Re = H * R0_0 + (1.0/3.0)*F_std*(R0_1 - R0_2) + 0.4*H*Tr_R0
    S_Im = H * I0_0 + (1.0/3.0)*F_std*(I0_1 - I0_2) + 0.4*H*Tr_I0 - tau * I0_0

    inv_den = 1.0 / (D_real**2 + D_imag**2 + 1e-25)

    prod_dot   = R0_0 * D_real + I0_0 * D_imag
    prod_cross = R0_0 * D_imag - I0_0 * D_real

    C_real = H * inv_den * prod_dot
    C_imag = -H * inv_den * prod_cross
    # -------------------------------------------------------------------------------------------------------------------

    dR0_0 = S_Re * C_real - S_Im * C_imag
    dI0_0 = S_Re * C_imag + S_Im * C_real
    
    zetaR = -(R0_0 * dR0_0 + I0_0 * dI0_0)/(calD * (R0_0**2 + I0_0**2 + eps))
    zetaI = -(R0_0 * dI0_0 - dR0_0 * I0_0)/(calD * (R0_0**2 + I0_0**2 + eps))
    
    C_term_real = -0.8 * F * R0_21
    C_term_imag = -0.8 * F * I0_21
    
    dR0_1 = ( - (0.6 * H) * R0_1 
              - ((0.4 * sqrt_k * term_cos_minus_1 * cos_phi) / denom) * R0_11 
              - ((0.8 * sqrt_k * term_cos_minus_1 * sin_phi) / denom) * R0_21 
              - ((0.4 * sqrt_k * term_1_minus_cos * cos_phi) / denom) * R0_22 
              - tau * R0_1 ) # RHS Real Part

    dR0_2 = ( - (0.6 * H) * R0_2 
              - ((0.4 * sqrt_k * term_1_minus_cos * sin_phi) / denom) * R0_11 
              - ((0.8 * sqrt_k * term_cos_minus_1 * cos_phi) / denom) * R0_12 
              - ((0.4 * sqrt_k * term_cos_minus_1 * sin_phi) / denom) * R0_22 
              - tau * R0_2 ) # RHS Real Part

    dR0_3 = ( - (0.6 * H) * R0_3 
              - ((0.4 * sqrt_k * term_cos_minus_1 * cos_phi) / denom) * R0_13 
              - ((0.4 * sqrt_k * term_cos_minus_1 * sin_phi) / denom) * R0_23 
              - tau * R0_3 ) # RHS Real Part

    dI0_1 = ( - (0.6 * H) * I0_1 
              - ((0.4 * sqrt_k * term_cos_minus_1 * cos_phi) / denom) * I0_11 
              - ((0.8 * sqrt_k * term_cos_minus_1 * sin_phi) / denom) * I0_21 
              - ((0.4 * sqrt_k * term_1_minus_cos * cos_phi) / denom) * I0_22 
              - tau * (2.0/3.0) * I0_1 )

    dI0_2 = ( - (0.6 * H) * I0_2 
              - ((0.4 * sqrt_k * term_1_minus_cos * sin_phi) / denom) * I0_11 
              - ((0.8 * sqrt_k * term_cos_minus_1 * cos_phi) / denom) * I0_12 
              - ((0.4 * sqrt_k * term_cos_minus_1 * sin_phi) / denom) * I0_22 
              - tau * (2.0/3.0) * I0_2 )

    dI0_3 = ( - (0.6 * H) * I0_3 
              - ((0.4 * sqrt_k * term_cos_minus_1 * cos_phi) / denom) * I0_13 
              - ((0.4 * sqrt_k * term_cos_minus_1 * sin_phi) / denom) * I0_23 
              - tau * (2.0/3.0) * I0_3 )

    dR0_11 = ( - ((0.333333333333333 * sqrt_k * term_1_minus_cos * cos_phi) / denom) * R0_1 
               - ((0.666666666666667 * sqrt_k * term_cos_minus_1 * sin_phi) / denom) * R0_2 
               - ( (0.444444444444444 * H) * R0_11 + (-0.222222222222222 * H) * R0_22 + (-0.222222222222222 * H) * R0_33 )
               - tau * (0.9 * R0_11 + 0.3 * R2_11) )

    dI0_11 = ( - ((0.333333333333333 * sqrt_k * term_1_minus_cos * cos_phi) / denom) * I0_1 
               - ((0.666666666666667 * sqrt_k * term_cos_minus_1 * sin_phi) / denom) * I0_2 
               - ( (0.444444444444444 * H) * I0_11 + (-0.222222222222222 * H) * I0_22 + (-0.222222222222222 * H) * I0_33 )
               - tau * I0_11 )

    dR0_12 = ( - ((0.5 * sqrt_k * term_1_minus_cos * sin_phi) / denom) * R0_1 
               - ((0.5 * sqrt_k * term_1_minus_cos * cos_phi) / denom) * R0_2 
               - ( (0.333333333333333 * H) * R0_12 + (0.333333333333333 * H) * R0_21 )
               - tau * (0.9 * R0_12 + 0.3 * R2_12) )

    dI0_12 = ( - ((0.5 * sqrt_k * term_1_minus_cos * sin_phi) / denom) * I0_1 
               - ((0.5 * sqrt_k * term_1_minus_cos * cos_phi) / denom) * I0_2 
               - ( (0.333333333333333 * H) * I0_12 + (0.333333333333333 * H) * I0_21 )
               - tau * I0_12 )

    dR0_13 = ( - ( (0.333333333333333 * H) * R0_13 + (0.333333333333333 * H) * R0_31 )
               - tau * (0.9 * R0_13 + 0.3 * R2_13) )

    dI0_13 = ( - ( (0.333333333333333 * H) * I0_13 + (0.333333333333333 * H) * I0_31 )
               - tau * I0_13 )

    dR0_21 = ( - ((0.5 * sqrt_k * term_1_minus_cos * sin_phi) / denom) * R0_1 
               - ((0.5 * sqrt_k * term_1_minus_cos * cos_phi) / denom) * R0_2 
               - ( (0.333333333333333 * H) * R0_21 + (0.333333333333333 * H) * R0_12 )
               - tau * (0.9 * R0_21 + 0.3 * R2_21) )

    dI0_21 = ( - ((0.5 * sqrt_k * term_1_minus_cos * sin_phi) / denom) * I0_1 
               - ((0.5 * sqrt_k * term_1_minus_cos * cos_phi) / denom) * I0_2 
               - ( (0.333333333333333 * H) * I0_21 + (0.333333333333333 * H) * I0_12 )
               - tau * I0_21 )

    dR0_22 = ( - ((0.666666666666667 * sqrt_k * term_cos_minus_1 * cos_phi) / denom) * R0_1 
               - ((0.333333333333333 * sqrt_k * term_1_minus_cos * sin_phi) / denom) * R0_2 
               - ( (-0.222222222222222 * H) * R0_11 + (0.444444444444444 * H) * R0_22 + (-0.222222222222222 * H) * R0_33 )
               - tau * (0.9 * R0_22 + 0.3 * R2_22) )

    dI0_22 = ( - ((0.666666666666667 * sqrt_k * term_cos_minus_1 * cos_phi) / denom) * I0_1 
               - ((0.333333333333333 * sqrt_k * term_1_minus_cos * sin_phi) / denom) * I0_2 
               - ( (-0.222222222222222 * H) * I0_11 + (0.444444444444444 * H) * I0_22 + (-0.222222222222222 * H) * I0_33 )
               - tau * I0_22 )

    dR0_23 = ( - ( (0.333333333333333 * H) * R0_23 + (0.333333333333333 * H) * R0_32 )
               - tau * (0.9 * R0_23 + 0.3 * R2_23) )

    dI0_23 = ( - ( (0.333333333333333 * H) * I0_23 + (0.333333333333333 * H) * I0_32 )
               - tau * I0_23 )

    dR0_31 = ( - ( (0.333333333333333 * H) * R0_31 + (0.333333333333333 * H) * R0_13 )
               - tau * (0.9 * R0_31 + 0.3 * R2_31) )

    dI0_31 = ( - ( (0.333333333333333 * H) * I0_31 + (0.333333333333333 * H) * I0_13 )
               - tau * I0_31 )

    dR0_32 = ( - ( (0.333333333333333 * H) * R0_32 + (0.333333333333333 * H) * R0_23 )
               - tau * (0.9 * R0_32 + 0.3 * R2_32) )

    dI0_32 = ( - ( (0.333333333333333 * H) * I0_32 + (0.333333333333333 * H) * I0_23 )
               - tau * I0_32 )

    dR0_33 = ( - ((0.333333333333333 * sqrt_k * term_1_minus_cos * cos_phi) / denom) * R0_1 
               - ((0.333333333333333 * sqrt_k * term_1_minus_cos * sin_phi) / denom) * R0_2 
               - ( (-0.222222222222222 * H) * R0_11 + (-0.222222222222222 * H) * R0_22 + (0.444444444444444 * H) * R0_33 )
               - tau * (0.9 * R0_33 + 0.3 * R2_33) )

    dI0_33 = ( - ((0.333333333333333 * sqrt_k * term_1_minus_cos * cos_phi) / denom) * I0_1 
               - ((0.333333333333333 * sqrt_k * term_1_minus_cos * sin_phi) / denom) * I0_2 
               - ( (-0.222222222222222 * H) * I0_11 + (-0.222222222222222 * H) * I0_22 + (0.444444444444444 * H) * I0_33 )
               - tau * I0_33)

    dR2_11 = ( - ( (0.444444444444444 * H) * R2_11 + (-0.222222222222222 * H) * R2_22 + (-0.222222222222222 * H) * R2_33 )
               - tau * (0.2 * R0_11 + 0.4 * R2_11) )

    dI2_11 = ( - ( (0.444444444444444 * H) * I2_11 + (-0.222222222222222 * H) * I2_22 + (-0.222222222222222 * H) * I2_33 )
               - tau * I2_11 )

    dR2_12 = ( - ( (0.333333333333333 * H) * R2_12 + (0.333333333333333 * H) * R2_21 )
               - tau * (0.2 * R0_12 + 0.4 * R2_12) )

    dI2_12 = ( - ( (0.333333333333333 * H) * I2_12 + (0.333333333333333 * H) * I2_21 )
               - tau * I2_12 )

    dR2_13 = ( - ( (0.333333333333333 * H) * R2_13 + (0.333333333333333 * H) * R2_31 )
               - tau * (0.2 * R0_13 + 0.4 * R2_13) )

    dI2_13 = ( - ( (0.333333333333333 * H) * I2_13 + (0.333333333333333 * H) * I2_31 )
               - tau * I2_13 )

    dR2_21 = ( - ( (0.333333333333333 * H) * R2_21 + (0.333333333333333 * H) * R2_12 )
               - tau * (0.2 * R0_21 + 0.4 * R2_21) )

    dI2_21 = ( - ( (0.333333333333333 * H) * I2_21 + (0.333333333333333 * H) * I2_12 )
               - tau * I2_21 )

    dR2_22 = ( - ( (-0.222222222222222 * H) * R2_11 + (0.444444444444444 * H) * R2_22 + (-0.222222222222222 * H) * R2_33 )
               - tau * (0.2 * R0_22 + 0.4 * R2_22) )

    dI2_22 = ( - ( (-0.222222222222222 * H) * I2_11 + (0.444444444444444 * H) * I2_22 + (-0.222222222222222 * H) * I2_33 )
               - tau * I2_22 )

    dR2_23 = ( - ( (0.333333333333333 * H) * R2_23 + (0.333333333333333 * H) * R2_32 )
               - tau * (0.2 * R0_23 + 0.4 * R2_23) )

    dI2_23 = ( - ( (0.333333333333333 * H) * I2_23 + (0.333333333333333 * H) * I2_32 )
               - tau * I2_23 )

    dR2_31 = ( - ( (0.333333333333333 * H) * R2_31 + (0.333333333333333 * H) * R2_13 )
               - tau * (0.2 * R0_31 + 0.4 * R2_31) )

    dI2_31 = ( - ( (0.333333333333333 * H) * I2_31 + (0.333333333333333 * H) * I2_13 )
               - tau * I2_31 )

    dR2_32 = ( - ( (0.333333333333333 * H) * R2_32 + (0.333333333333333 * H) * R2_23 )
               - tau * (0.2 * R0_32 + 0.4 * R2_32) )

    dI2_32 = ( - ( (0.333333333333333 * H) * I2_32 + (0.333333333333333 * H) * I2_23 )
               - tau * I2_32 )

    dR2_33 = ( - ( (-0.222222222222222 * H) * R2_11 + (-0.222222222222222 * H) * R2_22 + (0.444444444444444 * H) * R2_33 )
               - tau * (0.2 * R0_33 + 0.4 * R2_33) )

    dI2_33 = ( - ( (-0.222222222222222 * H) * I2_11 + (-0.222222222222222 * H) * I2_22 + (0.444444444444444 * H) * I2_33 )
               - tau * I2_33 )
...
if __name__ == "__main__":
    ...
    save_path = os.path.join(output_dir, "RS2.png")
    ...
    plt.title(r'Evolution of fluctuation amplitudes in $\mathbb{R} \times S^2$', fontsize=14)
    plt.legend()
    plt.grid(True, which="both", ls="--", alpha=0.5)
    
    amp_save_path = os.path.join(output_dir, "RS2_ampl.png")
