...
def derivatives_hpc(t, y):
    ...
    arg = 1.5 * np.sqrt(0.7) * t                                                                                         #Dimensionless time parameter 't' (= Physical time multilpied with H_0)
    ...
    d_theta = -K_fac * np.sin(theta) * (1.0 + np.cos(theta)**2) * (np.cos(phi)**2 - np.sin(phi)**2)
    d_phi   = (4.0 * K_fac) * np.cos(theta) * np.sin(phi) * np.cos(phi)
    ...
    term_C1   = (0.4 * K_fac * R0_31)
    term_C1_I = (0.4 * K_fac * I0_31)
    
    dR0_1 = - (0.6 * H_val * R0_1 + term_C1) - tau * R0_1
    dI0_1 = - (0.6 * H_val * I0_1 + term_C1_I) - tau * (2.0/3.0) * I0_1

    term_C2   = (-0.4 * K_fac * R0_32)
    term_C2_I = (-0.4 * K_fac * I0_32)

    dR0_2 = - (0.6 * H_val * R0_2 + term_C2) - tau * R0_2
    dI0_2 = - (0.6 * H_val * I0_2 + term_C2_I) - tau * (2.0/3.0) * I0_2

    term_C3   = (-0.4 * K_fac * R0_11) + (0.4 * K_fac * R0_22)
    term_C3_I = (-0.4 * K_fac * I0_11) + (0.4 * K_fac * I0_22)

    dR0_3 = - (0.6 * H_val * R0_3 + term_C3) - tau * R0_3
    dI0_3 = - (0.6 * H_val * I0_3 + term_C3_I) - tau * (2.0/3.0) * I0_3

    dR0_11 = - (1.0 * K_fac * R0_3) - tau * (0.9 * R0_11 + 0.3 * R2_11)
    dI0_11 = - (1.0 * K_fac * I0_3) - tau * I0_11

    dR0_22 = - (-1.0 * K_fac * R0_3) - tau * (0.9 * R0_22 + 0.3 * R2_22)
    dI0_22 = - (-1.0 * K_fac * I0_3) - tau * I0_22

    dR0_13 = - (-0.5 * K_fac * R0_1) - tau * (0.9 * R0_13 + 0.3 * R2_13)
    dI0_13 = - (-0.5 * K_fac * I0_1) - tau * I0_13

    dR0_23 = - (0.5 * K_fac * R0_2) - tau * (0.9 * R0_23 + 0.3 * R2_23)
    dI0_23 = - (0.5 * K_fac * I0_2) - tau * I0_23

    dR0_31 = - (-0.5 * K_fac * R0_1) - tau * (0.9 * R0_31 + 0.3 * R2_31)
    dI0_31 = - (-0.5 * K_fac * I0_1) - tau * I0_31

    dR0_32 = - (0.5 * K_fac * R0_2) - tau * (0.9 * R0_32 + 0.3 * R2_32)
    dI0_32 = - (0.5 * K_fac * I0_2) - tau * I0_32
    ...
    k_11_11 = 0.444444444 * H_val
    k_22_22 = 0.444444444 * H_val
    k_33_33 = 0.444444444 * H_val
    
    k_off_trace = -0.222222222 * H_val # e.g. K11_22
    
    k_sym_sum = (0.333333333 + 0.333333333) * H_val 

    sum_K_11_R = k_11_11 * R2_11 + k_off_trace * R2_22 + k_off_trace * R2_33
    sum_K_11_I = k_11_11 * I2_11 + k_off_trace * I2_22 + k_off_trace * I2_33
    
    dR2_11 = H_val * (zetaR * R2_11 - zetaI * I2_11) - sum_K_11_R - tau * (0.2 * R0_11 + 0.4 * R2_11)
    dI2_11 = H_val * (zetaR * I2_11 + zetaI * R2_11) - sum_K_11_I - tau * I2_11

    sum_K_22_R = k_off_trace * R2_11 + k_22_22 * R2_22 + k_off_trace * R2_33
    sum_K_22_I = k_off_trace * I2_11 + k_22_22 * I2_22 + k_off_trace * I2_33

    dR2_22 = H_val * (zetaR * R2_22 - zetaI * I2_22) - sum_K_22_R - tau * (0.2 * R0_22 + 0.4 * R2_22)
    dI2_22 = H_val * (zetaR * I2_22 + zetaI * R2_22) - sum_K_22_I - tau * I2_22

    sum_K_33_R = k_off_trace * R2_11 + k_off_trace * R2_22 + k_33_33 * R2_33
    sum_K_33_I = k_off_trace * I2_11 + k_off_trace * I2_22 + k_33_33 * I2_33

    dR2_33 = H_val * (zetaR * R2_33 - zetaI * I2_33) - sum_K_33_R - tau * (0.2 * R0_33 + 0.4 * R2_33)
    dI2_33 = H_val * (zetaR * I2_33 + zetaI * R2_33) - sum_K_33_I - tau * I2_33

    dR2_12 = H_val * (zetaR * R2_12 - zetaI * I2_12) - (k_sym_sum * R2_12) - tau * (0.2 * R0_12 + 0.4 * R2_12)
    dI2_12 = H_val * (zetaR * I2_12 + zetaI * R2_12) - (k_sym_sum * I2_12) - tau * I2_12

    dR2_21 = H_val * (zetaR * R2_21 - zetaI * I2_21) - (k_sym_sum * R2_21) - tau * (0.2 * R0_21 + 0.4 * R2_21)
    dI2_21 = H_val * (zetaR * I2_21 + zetaI * R2_21) - (k_sym_sum * I2_21) - tau * I2_21

    dR2_13 = H_val * (zetaR * R2_13 - zetaI * I2_13) - (k_sym_sum * R2_13) - tau * (0.2 * R0_13 + 0.4 * R2_13)
    dI2_13 = H_val * (zetaR * I2_13 + zetaI * R2_13) - (k_sym_sum * I2_13) - tau * I2_13

    dR2_31 = H_val * (zetaR * R2_31 - zetaI * I2_31) - (k_sym_sum * R2_31) - tau * (0.2 * R0_31 + 0.4 * R2_31)
    dI2_31 = H_val * (zetaR * I2_31 + zetaI * R2_31) - (k_sym_sum * I2_31) - tau * I2_31

    dR2_23 = H_val * (zetaR * R2_23 - zetaI * I2_23) - (k_sym_sum * R2_23) - tau * (0.2 * R0_23 + 0.4 * R2_23)
    dI2_23 = H_val * (zetaR * I2_23 + zetaI * R2_23) - (k_sym_sum * I2_23) - tau * I2_23

    dR2_32 = H_val * (zetaR * R2_32 - zetaI * I2_32) - (k_sym_sum * R2_32) - tau * (0.2 * R0_32 + 0.4 * R2_32)
    dI2_32 = H_val * (zetaR * I2_32 + zetaI * R2_32) - (k_sym_sum * I2_32) - tau * I2_32
