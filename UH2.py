...
def derivatives_hpc(t, y):
    ...
    H_val = Ha
    K_fac = np.sqrt(-k) / (c * a_t * 2.56e-18 + eps)                                                         		  #Dimensionless qty, since numerator is in sec inverse
    
    arg_hyp = (x_coord * sqrt_neg_k) / c
    tanh_x  = np.tanh(arg_hyp)
    cosh_x  = np.cosh(arg_hyp)
    cosh_3x = np.cosh(3.0 * arg_hyp)
    
    G_term = -1.5 + (0.375 / (cosh_x**2)) + (0.125 * cosh_3x / (cosh_x**3))
    K_imag_common = (sqrt_neg_k * G_term) / (3.0 * c * a_t + eps)

    C_huge_1 = (sqrt_neg_k * (0.15 * cosh_x + 0.05 * cosh_3x)) / (c * a_t * (cosh_x**3) + eps)
    C_huge_2 = (sqrt_neg_k * (-0.6 + (0.3/(cosh_x**2)) + (0.1 * cosh_3x/(cosh_x**3)))) / (c * a_t + eps)

    H_huge = (sqrt_neg_k * (-0.75 + (0.5625/(cosh_x**2)) + (0.1875 * cosh_3x/(cosh_x**3)))) / (c * a_t + eps)
      
    calD = Ha + eps
    
    d_theta = -((1.0 * np.sqrt(-k) * np.tanh(x * np.sqrt(-k) / c)) / (c * a_t + eps)) * (np.sin(theta)**2) * np.cos(theta) * (np.sin(phi)**2) * np.cos(phi)
    d_phi   = -((1.0 * np.sqrt(-k) * np.tanh(x * np.sqrt(-k) / c)) / (c * a_t + eps)) * np.sin(theta) * np.sin(phi) * (1.0 + (np.cos(phi)**2)) - ((1.0 * np.sqrt(-k)) / (c * a_t + eps)) * np.cos(theta)
    d_psi_real = 0
    d_psi_imag = 0
    ...
    S_Re = -H_val * R0_0 + (2.0/3.0) * K_fac * R0_1 - 0.4 * H_val * tr_R0
    S_Im = -H_val * I0_0 + (2.0/3.0) * K_fac * I0_1 - 0.4 * H_val * tr_I0 - (1.0/L) * I0_0

    D_real = -H_val * R0_0 - (2.0/15.0) * H_val * tr_R0
    D_imag = -H_val * I0_0 - (2.0/15.0) * H_val * tr_I0
    
    D_minus_N_Re = D_real - R0_0
    D_minus_N_Im = D_imag - I0_0

    inv_D_sq = 1.0 / (D_minus_N_Re**2 + D_minus_N_Im**2 + eps**2)
    
    C_Re = (D_real * D_minus_N_Re + D_imag * D_minus_N_Im) * inv_D_sq
    C_Im = (D_imag * D_minus_N_Re - D_real * D_minus_N_Im) * inv_D_sq
    # -------------------------------------------------------------------------------------------------------------------
    ...
    term_C1 = (0.4 * K_fac * tanh_x * R0_11) + (-0.4 * K_fac * tanh_x * R0_22) + \
              (-0.2 * K_fac * R0_23) + (C_huge_2 * R0_32)
    term_C1_I = (0.4 * K_fac * tanh_x * I0_11) + (-0.4 * K_fac * tanh_x * I0_22) + \
                (-0.2 * K_fac * I0_23) + (C_huge_2 * I0_32)
                
    dR0_1 = - (0.6 * H_val * (zetaR * R0_1 - zetaI * I0_1) + term_C1) - tau * R0_1
    dI0_1 = - (0.6 * H_val * (zetaR * I0_1 + zetaI * R0_1) + term_C1_I) - tau * (2.0/3.0) * I0_1

    term_C2 = (0.8 * K_fac * tanh_x * R0_12) + (0.2 * K_fac * R0_13) + (C_huge_1 * R0_31)
    term_C2_I = (0.8 * K_fac * tanh_x * I0_12) + (0.2 * K_fac * I0_13) + (C_huge_1 * I0_31)

    dR0_2 = - (0.6 * H_val * (zetaR * R0_2 - zetaI * I0_2) + term_C2) - tau * R0_2
    dI0_2 = - (0.6 * H_val * (zetaR * I0_2 + zetaI * R0_2) + term_C2_I) - tau * (2.0/3.0) * I0_2

    term_C3 = (0.2 * K_fac * R0_12) + (0.4 * K_fac * tanh_x * R0_13) + (-0.2 * K_fac * R0_21)
    term_C3_I = (0.2 * K_fac * I0_12) + (0.4 * K_fac * tanh_x * I0_13) + (-0.2 * K_fac * I0_21)

    dR0_3 = - (0.6 * H_val * (zetaR * R0_3 - zetaI * I0_3) + term_C3) - tau * R0_3
    dI0_3 = - (0.6 * H_val * (zetaR * I0_3 + zetaI * R0_3) + term_C3_I) - tau * (2.0/3.0) * I0_3

    dR0_11 = - (-0.333333333 * K_fac * tanh_x * R0_1) - tau * (0.9 * R0_11 + 0.3 * R2_11)
    dI0_11 = - (-0.333333333 * K_fac * tanh_x * I0_1) - tau * I0_11

    dR0_12 = - ( (-0.5 * K_fac * tanh_x * R0_2) + (H_huge * R0_3) ) - tau * (0.9 * R0_12 + 0.3 * R2_12)
    dI0_12 = - ( (-0.5 * K_fac * tanh_x * I0_2) + (H_huge * I0_3) ) - tau * I0_12

    dR0_13 = - (-0.5 * K_fac * R0_2) - tau * (0.9 * R0_13 + 0.3 * R2_13)
    dI0_13 = - (-0.5 * K_fac * I0_2) - tau * I0_13

    dR0_21 = - ( (-0.5 * K_fac * tanh_x * R0_2) + (H_huge * R0_3) ) - tau * (0.9 * R0_21 + 0.3 * R2_21)
    dI0_21 = - ( (-0.5 * K_fac * tanh_x * I0_2) + (H_huge * I0_3) ) - tau * I0_21

    dR0_22 = - (0.666666667 * K_fac * tanh_x * R0_1) - tau * (0.9 * R0_22 + 0.3 * R2_22)
    dI0_22 = - (0.666666667 * K_fac * tanh_x * I0_1) - tau * I0_22

    dR0_23 = - (0.5 * K_fac * R0_1) - tau * (0.9 * R0_23 + 0.3 * R2_23)
    dI0_23 = - (0.5 * K_fac * I0_1) - tau * I0_23

    dR0_31 = - (-0.5 * K_fac * R0_2) - tau * (0.9 * R0_31 + 0.3 * R2_31)
    dI0_31 = - (-0.5 * K_fac * I0_2) - tau * I0_31

    dR0_32 = - (0.5 * K_fac * R0_1) - tau * (0.9 * R0_32 + 0.3 * R2_32)
    dI0_32 = - (0.5 * K_fac * I0_1) - tau * I0_32

    dR0_33 = - (-0.333333333 * K_fac * tanh_x * R0_1) - tau * (0.9 * R0_33 + 0.3 * R2_33)
    dI0_33 = - (-0.333333333 * K_fac * tanh_x * I0_1) - tau * I0_33

    def calc_KN(Kr, Ki, Nr, Ni):
        res_r = Kr * Nr - Ki * Ni
        res_i = Kr * Ni + Ki * Nr
        return res_r, res_i

    Kr_133 = 0.444444444 * H_val
    Kr_100 = 0.333333333 * H_val
    Kr_neg022 = -0.222222222 * H_val 
    
    kn_11_r, kn_11_i = calc_KN(Kr_133, K_imag_common, R2_11, I2_11)
    kn_22_r, kn_22_i = calc_KN(Kr_neg022, 0.0, R2_22, I2_22)
    kn_33_r, kn_33_i = calc_KN(Kr_neg022, 0.0, R2_33, I2_33)
    
    dR2_11 = H_val * (zetaR * R2_11 - zetaI * I2_11) - (kn_11_r + kn_22_r + kn_33_r) - tau * (0.2 * R0_11 + 0.4 * R2_11)
    dI2_11 = H_val * (zetaR * I2_11 + zetaI * R2_11) - (kn_11_i + kn_22_i + kn_33_i) - tau * I2_11

    kn_12_r, kn_12_i = calc_KN(Kr_100, K_imag_common, R2_12, I2_12)
    kn_21_r, kn_21_i = calc_KN(Kr_100, 0.0, R2_21, I2_21)

    dR2_12 = H_val * (zetaR * R2_12 - zetaI * I2_12) - (kn_12_r + kn_21_r) - tau * (0.2 * R0_12 + 0.4 * R2_12)
    dI2_12 = H_val * (zetaR * I2_12 + zetaI * R2_12) - (kn_12_i + kn_21_i) - tau * I2_12

    kn_13_r, kn_13_i = calc_KN(Kr_100, K_imag_common, R2_13, I2_13)
    kn_31_r, kn_31_i = calc_KN(Kr_100, 0.0, R2_31, I2_31)

    dR2_13 = H_val * (zetaR * R2_13 - zetaI * I2_13) - (kn_13_r + kn_31_r) - tau * (0.2 * R0_13 + 0.4 * R2_13)
    dI2_13 = H_val * (zetaR * I2_13 + zetaI * R2_13) - (kn_13_i + kn_31_i) - tau * I2_13

    kn_12_r_21, kn_12_i_21 = calc_KN(Kr_100, 0.0, R2_12, I2_12)
    kn_21_r_21, kn_21_i_21 = calc_KN(Kr_100, K_imag_common, R2_21, I2_21)

    dR2_21 = H_val * (zetaR * R2_21 - zetaI * I2_21) - (kn_12_r_21 + kn_21_r_21) - tau * (0.2 * R0_21 + 0.4 * R2_21)
    dI2_21 = H_val * (zetaR * I2_21 + zetaI * R2_21) - (kn_12_i_21 + kn_21_i_21) - tau * I2_21

    kn_11_r_22, kn_11_i_22 = calc_KN(Kr_neg022, 0.0, R2_11, I2_11)
    kn_22_r_22, kn_22_i_22 = calc_KN(Kr_133, K_imag_common, R2_22, I2_22)
    kn_33_r_22, kn_33_i_22 = calc_KN(Kr_neg022, 0.0, R2_33, I2_33)

    dR2_22 = H_val * (zetaR * R2_22 - zetaI * I2_22) - (kn_11_r_22 + kn_22_r_22 + kn_33_r_22) - tau * (0.2 * R0_22 + 0.4 * R2_22)
    dI2_22 = H_val * (zetaR * I2_22 + zetaI * R2_22) - (kn_11_i_22 + kn_22_i_22 + kn_33_i_22) - tau * I2_22

    kn_23_r, kn_23_i = calc_KN(Kr_100, K_imag_common, R2_23, I2_23)
    kn_32_r, kn_32_i = calc_KN(Kr_100, 0.0, R2_32, I2_32)

    dR2_23 = H_val * (zetaR * R2_23 - zetaI * I2_23) - (kn_23_r + kn_32_r) - tau * (0.2 * R0_23 + 0.4 * R2_23)
    dI2_23 = H_val * (zetaR * I2_23 + zetaI * R2_23) - (kn_23_i + kn_32_i) - tau * I2_23

    kn_13_r_31, kn_13_i_31 = calc_KN(Kr_100, 0.0, R2_13, I2_13)
    kn_31_r_31, kn_31_i_31 = calc_KN(Kr_100, K_imag_common, R2_31, I2_31)

    dR2_31 = H_val * (zetaR * R2_31 - zetaI * I2_31) - (kn_13_r_31 + kn_31_r_31) - tau * (0.2 * R0_31 + 0.4 * R2_31)
    dI2_31 = H_val * (zetaR * I2_31 + zetaI * R2_31) - (kn_13_i_31 + kn_31_i_31) - tau * I2_31

    kn_23_r_32, kn_23_i_32 = calc_KN(Kr_100, 0.0, R2_23, I2_23)
    kn_32_r_32, kn_32_i_32 = calc_KN(Kr_100, K_imag_common, R2_32, I2_32)

    dR2_32 = H_val * (zetaR * R2_32 - zetaI * I2_32) - (kn_23_r_32 + kn_32_r_32) - tau * (0.2 * R0_32 + 0.4 * R2_32)
    dI2_32 = H_val * (zetaR * I2_32 + zetaI * R2_32) - (kn_23_i_32 + kn_32_i_32) - tau * I2_32

    kn_11_r_33, kn_11_i_33 = calc_KN(Kr_neg022, 0.0, R2_11, I2_11)
    kn_22_r_33, kn_22_i_33 = calc_KN(Kr_neg022, 0.0, R2_22, I2_22)
    kn_33_r_33, kn_33_i_33 = calc_KN(Kr_133, K_imag_common, R2_33, I2_33)

    dR2_33 = H_val * (zetaR * R2_33 - zetaI * I2_33) - (kn_11_r_33 + kn_22_r_33 + kn_33_r_33) - tau * (0.2 * R0_33 + 0.4 * R2_33)
    dI2_33 = H_val * (zetaR * I2_33 + zetaI * R2_33) - (kn_11_i_33 + kn_22_i_33 + kn_33_i_33) - tau * I2_33
...
if __name__ == "__main__":
    ...
    plt.title(r'Evolution of fluctuation amplitudes in $\widetilde{U \left(\mathbb{H}^2 \right)}$', fontsize=14)
    plt.legend()
    plt.grid(True, which="both", ls="--", alpha=0.5)
    
    amp_save_path = os.path.join(output_dir, "UH2_ampl.png")
